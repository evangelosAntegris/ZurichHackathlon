...
export async function classifierA(routed: RoutedOut, cfg: PipelineConfig): Promise<ClassifierOut> {
  const prompt = await loadPrompt(cfg.classification.promptAPath)
  ...
  let modelId = cfg.models.light

  try {
    const providerResult = await callProvider(cfg, "light", {
      messages,
      expectJson: true,
      temperature: 0,
      maxTokens: 600,
    ...
    console.warn("[classifierA] Falling back to triggers:", error)
  }
