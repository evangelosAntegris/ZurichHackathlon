/**
 * Returns a raw TXT prompt from a constant string.
 * Useful for environments where fs/promises is not available.
 */

const PROMPT_TXT = `You are a Swiss banking assistant. Read the transcript and assign multi-label tasks strictly from ["plan_contact","schedule_meeting","update_contact_info_non_postal","update_contact_info_postal_address","update_kyc_activity","update_kyc_origin_of_assets","update_kyc_purpose_of_businessrelation","update_kyc_total_assets"].
...
export async function loadTxtPrompt(): Promise<string> {
  return PROMPT_TXT;
}
